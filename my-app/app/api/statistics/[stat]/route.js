...
async function getMostPopularProducts() {
	...
	mostPopularProducts.sort((a, b) => {
		const quantitySoldA = a.Purchase.reduce(
			(total, purchase) => total + purchase.quantity,
			0
		);
		const quantitySoldB = b.Purchase.reduce(
			(total, purchase) => total + purchase.quantity,
			0
		);
	...
	return mostPopularProducts.map((product) => ({
		name: product.name,
		image: product.image,
		quantitySold: product.Purchase.reduce(
			(total, purchase) => total + purchase.quantity,
			0
		),
		moneyMade: product.Purchase.reduce(
			(total, purchase) => total + purchase.quantity * product.price,
			0
		),
	}));
}
