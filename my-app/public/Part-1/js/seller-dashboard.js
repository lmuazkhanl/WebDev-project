...
async function fetchQuantitySold(productId) {
	try {
		const response = await fetch(
			`http://localhost:3000/api/products/${productId}?stat=quantity`
		);
...
async function fetchCustomerList(productId) {
	try {
		const response = await fetch(
			`http://localhost:3000/api/products/${productId}?stat=customers`
		);
		const customerList = await response.json();
		return customerList.map(
			(customer) => `<li class="customer-name">${customer}</li>`
		);
	} catch (error) {
