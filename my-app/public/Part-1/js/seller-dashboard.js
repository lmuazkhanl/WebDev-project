...
function showPage(pageName) {
	...
	if (selectedPage) {
		...
		if (selectedPage === document.querySelector(".Current-Catalogue-Page"))
			displaySellerItems();
		if (selectedPage === document.querySelector(".Sale-History-Page"))
			displaySaleHistory();
	}
}
...
async function updateItem(itemName, newQuantity) {
...
async function displaySaleHistory() {
	...
	const productGridSaleHistory = document.querySelector(
...
function generateSaleHistoryCard(items) {
	return items.map(
        ...
        <div class="sale-history-product-card">
            ...
            <div class="product-details">
                <h2 class="product-name">${item.name}</h2>
                <p class="price-text">Sold at <span class="price">$${
									item.price
								}</span></p>
                <p class="stock-remaining-text">Stock Remaining : <span class="quantity">${
									item.quantity
								}</span></p>
                <p class="stock-sold-text">Items Sold : <span class="quantity-sold">${calculateQuantitySold(
									item
								)}</span></p>
...
function calculateQuantitySold(item) {
	console.log(item);
	return item.purchaseHistory.reduce(
...
function generateCustomerList(item) {
	return item.purchaseHistory
		.map((purchase) => `<li class="customer-name">${purchase.customer}</li>`)
		.join("");
}
...
document.querySelector(
	".seller_Name"
).innerHTML = `${sellerName} (Balance: $${sellerBalance.toFixed(2)})`;

const usersData = JSON.parse(localStorage.getItem("users"));

const sellerIndex = usersData.sellers.findIndex(
